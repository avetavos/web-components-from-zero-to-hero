---
title: 'โหมด Open และ Closed'
description: 'ความแตกต่างระหว่าง mode open และ mode closed ใน attachShadow และข้อดีข้อเสีย'
sidebar:
  order: 4
---

import LivePreview from '../../../../components/LivePreview.tsx';
import Tip from '../../../../components/Tip.astro';
import Quiz from '../../../../components/Quiz.tsx';
import ProgressTracker from '../../../../components/ProgressTracker.tsx';

## โหมด open กับ closed

เมื่อเรียก `attachShadow` คุณต้องระบุ `mode` เป็น `'open'` หรือ `'closed'` ซึ่งกำหนดว่า JavaScript ภายนอกจะเข้าถึง shadow root ผ่าน `.shadowRoot` property ได้หรือไม่

| โหมด | `element.shadowRoot` คืนค่า | ใช้เมื่อ |
|---|---|---|
| `open` | shadow root object | ทั่วไป — DevTools, tests, Lit ต้องการ |
| `closed` | `null` | ต้องการซ่อน internal จาก JS ภายนอก |

## โหมด open

ด้วยโหมด `open` shadow root จะเข้าถึงได้จากภายนอก component:

```js
class OpenBox extends HTMLElement {
  connectedCallback() {
    this.attachShadow({ mode: 'open' });
    this.shadowRoot.innerHTML = '<p>Open shadow root</p>';
  }
}
customElements.define('open-box', OpenBox);

var el = document.querySelector('open-box');
console.log(el.shadowRoot); // ShadowRoot { ... }
```

ซึ่งทำให้ DevTools, test frameworks และ libraries อย่าง Lit สามารถ inspect และ interact กับ shadow tree ได้

## โหมด closed

ด้วยโหมด `closed` property `.shadowRoot` จะคืนค่า `null` สำหรับ JavaScript ภายนอก:

```js
class ClosedBox extends HTMLElement {
  connectedCallback() {
    var root = this.attachShadow({ mode: 'closed' });
    // store root internally if you still need it
    root.innerHTML = '<p>Closed shadow root</p>';
  }
}
customElements.define('closed-box', ClosedBox);

var el = document.querySelector('closed-box');
console.log(el.shadowRoot); // null
```

shadow root ยังคงอยู่ภายใน — component เองก็ยังเข้าถึงได้ผ่าน variable ที่เก็บค่าที่ `attachShadow` คืนมา แต่โค้ดภายนอกไม่สามารถเข้าถึงได้ผ่าน `.shadowRoot`

## ตัวอย่างที่รันได้

export const openClosedJs = 'class OpenBox extends HTMLElement {\n' +
  '  connectedCallback() {\n' +
  '    this.attachShadow({ mode: \'open\' });\n' +
  '    this.shadowRoot.innerHTML =\n' +
  '      \'<style>div { background: #d1fae5; border: 2px solid #059669; padding: 0.75rem; border-radius: 6px; margin-bottom: 0.5rem; }</style>\' +\n' +
  '      \'<div>Open box (shadow visible externally)</div>\';\n' +
  '  }\n' +
  '}\n' +
  'customElements.define(\'open-box\', OpenBox);\n' +
  '\n' +
  'class ClosedBox extends HTMLElement {\n' +
  '  connectedCallback() {\n' +
  '    var root = this.attachShadow({ mode: \'closed\' });\n' +
  '    root.innerHTML =\n' +
  '      \'<style>div { background: #fee2e2; border: 2px solid #dc2626; padding: 0.75rem; border-radius: 6px; }</style>\' +\n' +
  '      \'<div>Closed box (shadow hidden externally)</div>\';\n' +
  '  }\n' +
  '}\n' +
  'customElements.define(\'closed-box\', ClosedBox);\n' +
  '\n' +
  'document.addEventListener(\'DOMContentLoaded\', function() {\n' +
  '  var openRoot = document.querySelector(\'open-box\').shadowRoot;\n' +
  '  var closedRoot = document.querySelector(\'closed-box\').shadowRoot;\n' +
  '  document.getElementById(\'out\').textContent =\n' +
  '    \'open-box.shadowRoot: \' + openRoot + \'\\n\' +\n' +
  '    \'closed-box.shadowRoot: \' + closedRoot;\n' +
  '});';

export const openClosedHtml = '<open-box></open-box><closed-box></closed-box><pre id="out"></pre>';

<LivePreview client:visible html={openClosedHtml} js={openClosedJs} />

<Tip title="ใช้โหมด open เป็นค่าเริ่มต้น">
ใช้ `mode: 'open'` สำหรับ component ที่ต้องการให้โค้ดอื่น (DevTools, tests, Lit) ตรวจสอบได้ โหมด closed ไม่ได้ให้ความปลอดภัยจริง และใช้งานได้ยากกว่า
</Tip>

## ข้อแลกเปลี่ยน

| ตัวเลือก | Benefit | Cost |
|---|---|---|
| `mode: 'open'` | DevTools, test frameworks และ library อย่าง Lit เข้าถึง `shadowRoot` ได้ตามปกติ ใช้งานง่าย debug ง่าย | internal structure เข้าถึงได้จาก JavaScript ภายนอกทั้งหมด ไม่มีการซ่อนจริง |
| `mode: 'closed'` | `.shadowRoot` คืนค่า `null` จากภายนอก ให้ความรู้สึกว่า internal ถูกซ่อน | เขียน test และ debug ยากขึ้นมาก เครื่องมือ inspect มาตรฐานเข้าไม่ถึง และไม่ได้เป็น security boundary จริง |

## ข้อผิดพลาดที่พบบ่อย

- **เลือก `mode: 'closed'` เพราะคิดว่าปลอดภัยกว่า** — closed mode แค่ซ่อน reference จาก `.shadowRoot` แต่ยังตรวจสอบ DOM ผ่าน `document.querySelector` และ dev tools บางส่วนได้อยู่ดี ไม่ใช่ security boundary จริง อย่าใช้แทนการป้องกันข้อมูลจริง
- **ใช้ `mode: 'closed'` แล้วลืมเก็บ reference ของ shadow root ไว้เอง** — เพราะ `this.shadowRoot` จะเป็น `null` ต้องเก็บค่าที่ `attachShadow` คืนมาไว้ใน variable หรือ private field ของ instance เอง
- **เลือก closed mode โดยไม่คิดถึง tooling** — test frameworks, DevTools inspector และ library หลายตัว (เช่น Lit) คาดหวัง `mode: 'open'` การเลือก closed โดยไม่จำเป็นทำให้ debug และ test component ยากขึ้นโดยไม่ได้อะไรกลับมาคุ้มค่า

> 💡 ตัวอย่างจากของจริง
>
> **Salesforce Lightning (LWC)** — เลือกใช้ `mode: 'open'` เป็นค่าเริ่มต้นสำหรับ component เพื่อให้ DevTools และ automated test เข้าถึง shadow tree ได้ในระดับ enterprise ([salesforce/lwc](https://github.com/salesforce/lwc))

export const quizOpenVsClosed = [
  {
    q: '`element.shadowRoot` คืนค่าอะไรเมื่อแนบ shadow root ด้วย `mode: "closed"`?',
    options: [
      'ออบเจ็กต์ ShadowRoot',
      'สตริงว่าง',
      'undefined',
      'null',
    ],
    answer: 3,
  },
  {
    q: 'โหมดใดที่แนะนำสำหรับคอมโพเนนต์ที่ต้องทำงานร่วมกับ DevTools ของ browser และเฟรมเวิร์กทดสอบ?',
    options: [
      'mode: "closed"',
      'mode: "open"',
      'mode: "public"',
      'mode: "dev"',
    ],
    answer: 1,
  },
  {
    q: 'เมื่อใช้ `mode: "closed"` จะเก็บ reference ไปยัง shadow root ไว้ใช้ภายในคอมโพเนนต์ได้อย่างไร?',
    options: [
      'ใช้ this.shadowRoot — ยังใช้งานได้จากภายใน',
      'เก็บค่าที่ attachShadow คืนมาไว้ในตัวแปร local หรือ instance',
      'ใช้ document.getShadowRoot(this)',
      'อิลิเมนต์โหมด closed ไม่มี shadow root — เนื้อหาอยู่ใน light DOM',
    ],
    answer: 1,
  },
];

<Quiz client:visible id="shadow-dom/open-vs-closed" questions={quizOpenVsClosed} />

<ProgressTracker client:visible id="shadow-dom/open-vs-closed" />
