...
- **คาดหวังว่า descendant selector ใน `::slotted()` จะทำงาน** — เขียน `::slotted(p) span {}` แล้วคาดว่าจะจัดสไตล์ `span` ที่ซ้อนอยู่ใน `p` ที่ถูก slot ได้ แต่ไวยากรณ์นี้ **ไม่ถูกต้อง** — `::slotted()` match ได้แค่ตัว element ที่ถูก slot โดยตรงระดับบนสุดเท่านั้น ไม่ใช่ descendant ของมัน
